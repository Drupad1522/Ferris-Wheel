...
#define WHEEL 1
#define CABIN 2
bool isSunLight = true;  // Initially set to sun light mode
...
void draw_scene() {
    // Set lighting properties based on mode
    if (isSunLight) {
	...
	glPushMatrix();
	glColor3f(0.0, 0.4, 0.0);   //change color (base of ferris wheel)
	glTranslatef(0, -40, 0);
	glScalef(150, 2, 150);
	glutSolidCube(4);
	glPopMatrix();
